<x-app-layout>
    ...
    @php
        ...
        $savedColumns = session('visible_columns_types', array_keys($columns));
    @endphp
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 flex justify-between items-center flex-wrap gap-2">
                    <div class="flex gap-2">
                        ...
                        <div class = "transition ease-in cursor-not-allowed bg-gray-800 text-gray-200 px-4 py-2 rounded text-sm"><i class="fa-solid fa-layer-group pr-1"></i> Types</div>
                    </div>
            ...
            <div class="bg-white overflow-x-auto shadow-md sm:rounded-lg">
                <div class="p-4 border-b flex justify-between items-center">
                    <div class="flex items-center space-x-2">
                        <div class="text-lg font-semibold text-gray-800">Asset Type Records</div>
                        <form action="{{ route('types.index') }}" method="GET" class="flex items-center ml-4">
                    ...
                    <div class="flex justify-center items-center space-x-2">
                        ...
                        <a href="{{ route('types.create') }}" class="transition ease-in bg-green-700 hover:bg-green-600 text-white text-sm px-4 py-2 rounded hover:shadow-lg">
                            ...
                            <tr class="group hover:bg-gray-50 divide-x divide-gray-100">
                                ...
                                <td class="sticky right-0 px-4 py-2 text-right bg-gray-700">
                                    <div class="invisible group-hover:visible flex justify-start space-x-2">
                                        <a href="{{ route('types.edit', $type->type) }}" class="bg-gray-600 hover:bg-blue-600 text-blue-400 hover:text-white px-2 py-1 rounded">
                                            <i class="fa-solid fa-pen"></i>
                                        </a>
                                        <form action="{{ route('types.destroy', $type->type) }}" method="POST" onsubmit="return confirm('Delete this type?');">
                                            @csrf
                                            @method('DELETE')
            ...
            checkboxes.forEach(checkbox => {
                checkbox.addEventListener('change', function () {
                    ...
                    fetch("{{ route('settings.save-columns') }}", {
                        ...
                        body: JSON.stringify({ columns: selected, table: 'types' })
                    });
                });
            });
        });
